...
type ChannelOtherSettings struct {
	...
	// QuotaQueryCredentialChannelID references another channel whose Key holds
	// the dedicated quota-query credential (e.g. a volcengine AK/SK JSON or a
	// ZenMux management key). The ID is not secret; the referenced key is only
	// read server-side during a query or binding validation and is never
	// returned to clients.
	QuotaQueryCredentialChannelID         *int                              `json:"quota_query_credential_channel_id,omitempty"`
