...
func FetchUpstreamRatios(c *gin.Context) {
	...
	for _, chn := range upstreams {
		...
		go func(chItem dto.UpstreamDTO) {
			...
			data, err := fetchSingleUpstreamPricing(c.Request.Context(), chItem.BaseURL, chItem.Endpoint, chItem.ID, req.Timeout)
			if err != nil {
				logger.LogWarn(c.Request.Context(), "fetch upstream pricing failed from "+chItem.Name+": "+err.Error())
