...
// GetChannelQuotaUsageAllKeys returns the per-key plan usage of a multi-key
// channel. The optional ?key_indexes=0,1,2 parameter (repeatable and/or
// comma-separated) restricts the scan; nil means every key. Malformed values
// and out-of-range indexes are rejected with HTTP 400 before any upstream
// work. No aggregation happens here: each key's windows are returned as-is.
func GetChannelQuotaUsageAllKeys(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil || id <= 0 {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "invalid channel id"})
		return
	}
	keyIndexes, parseErr := service.ParseChannelKeyIndexes(c.QueryArray("key_indexes"))
	if parseErr != nil {
	...
	result := service.QueryChannelQuotaAllKeys(c.Request.Context(), channel, keyIndexes)
	c.JSON(http.StatusOK, gin.H{"success": true, "data": result})
}
